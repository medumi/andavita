...
flower=Image.open(Path(__file__).with_name('flower.png')).convert('RGBA')
def bloom(layer,cx,cy,size,t):
    size=max(1,int(size*(.92+.08*ease(t/.9))))
    icon=flower.resize((size,size),Image.Resampling.LANCZOS)
    layer.alpha_composite(icon,(int(cx-size/2),int(cy-size/2)))
...
def frame(kind,t,duration,preview=False):
    ...
    if lower:
        ...
        bloom(layer,1695,918,94,t)
        d.text((1740,1010),'andavita',font=label,fill=cream,anchor='rs')
    elif kind=='sicherheit':
        bloom(layer,960,145,105,t)
        textcenter(d,cfg['safety']['title'],260,serif,cream)
        for line,ypos in zip(cfg['safety']['lines'],[420,490,600,660,790]):
            textcenter(d,line,ypos,font('Arial.ttf',38),cream)
        textcenter(d,'ANDAVITA · BEWEGUNG & RUHE',955,label,cream)
    else:
        ink=teal if kind=='intro' else cream
        y=int(24*(1-ease(t/.9)))
        bloom(layer,960,210+y,155,t)
        textcenter(d,'andavita',340+y,bold,ink)
    ...
    return im
for kind,duration in [('intro',5),('outro',6),('bauchbinde',8),('sicherheit',22)]:
...
subprocess.run(["ffmpeg","-y","-hide_banner","-loglevel","error","-i",str(out/"intro.mp4"),"-i",str(out/"sicherheit.mp4"),"-filter_complex","[0:v][1:v]concat=n=2:v=1:a=0[v]","-map","[v]","-c:v","libx264","-preset","fast","-crf","20","-pix_fmt","yuv420p","-movflags","+faststart",str(out/"intro-sicherheit.mp4")],check=True)
